...
// AcceptAPISpec.
func (spec *AcceptAPISpec) AppliesTo(log logr.Logger, obj *unstructured.Unstructured) bool {
	...
	for _, filter := range spec.Filters {
		...
		if len(filter.ValueIn) > 0 {
			log.Info("Filter valueIn check", "val", val,
				"slices.Contains", slices.Contains(filter.ValueIn, val))
		...
		if filter.Boundary.Min != nil && *filter.Boundary.Min >= 0 {
			...
			log.Info("Filter boundary min check", "val", val, "numVal", numVal, "min", *filter.Boundary.Min)
		...
		if filter.Boundary.Max != nil && *filter.Boundary.Max >= 0 {
			numVal, err := strconv.Atoi(val)
			log.Info("Filter boundary max check", "val", val, "numVal", numVal, "max", *filter.Boundary.Max)
